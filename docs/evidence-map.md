...
| Separate delegate identity and grantor credentials | `src/evaluation.ts`, `src/delegations.ts` | Core substitution/inactivity tests; delegated API/SDK lifecycle tests |
...
| Policy-pinned delegation and binding hash | `src/delegations.ts`, `src/policy.ts` | Core policy-content/binding-tamper tests |
...
| Human-only exact-scope step-up | `src/step-up.ts` | Core human/type/capability/action/resource tests; API/SDK/UI paths |
