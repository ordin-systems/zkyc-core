...
import { createReferenceApp, type ReferenceIdKind } from "../src/app.js";
...
function validateExpectation(op: Operation, value: unknown, label: string): void {
  ...
  if (expectation.receipt !== undefined) {
    ...
    const lastAttempt = record(receipt.lastAttempt, `${label}.receipt.lastAttempt`);
    const outcome = text(lastAttempt.outcome, `${label}.receipt.lastAttempt.outcome`);
    if (outcome === "NONE") {
      exact(lastAttempt, ["outcome"], [], `${label}.receipt.lastAttempt`);
    } else {
      assert.ok(outcome === "ACCEPTED" || outcome === "REJECTED", `${label}.receipt.lastAttempt.outcome is invalid`);
      exact(lastAttempt, ["outcome", "reasonCode"], [], `${label}.receipt.lastAttempt`);
      const reasonCode = text(lastAttempt.reasonCode, `${label}.receipt.lastAttempt.reasonCode`);
      if (outcome === "ACCEPTED") {
...
async function apiRequest(
  ...
  return { status: response.status, body: record(await response.json(), "API response") };
}
...
async function executeTranscript(transcript: LifecycleTranscript): Promise<readonly Operation[]> {
  ...
  for (const step of transcript.steps) {
    ...
    switch (step.op) {
      case "issueCredential":
        actual = await apiRequest(app, "/credentials", "POST", input);
      ...
      case "issueDelegation": {
        ...
        actual = await apiRequest(app, "/delegations", "POST", {
      ...
      case "revokeCredential": {
        ...
        actual = await apiRequest(app, `/credentials/${encodeURIComponent(credential.id)}/revoke`, "POST", { reason: input.reason });
        break;
      }
      case "revokeDelegation": {
        const delegation = resolveReference(input.delegation, handles) as { readonly id: string };
        actual = await apiRequest(app, `/delegations/${encodeURIComponent(delegation.id)}/revoke`, "POST", { reason: input.reason });
        break;
      }
      case "evaluate": {
        if (input.authorityMode === "DIRECT") {
          const credential = resolveReference(input.credential, handles) as Credential;
          actual = await apiRequest(app, "/evaluations", "POST", {
            ...input,
            principal: principalFor(credential),
            credential,
          });
        } else {
          const delegateIdentityCredential = resolveReference(input.delegateIdentityCredential, handles) as Credential;
          const grantorCredential = resolveReference(input.grantorCredential, handles) as Credential;
          const delegation = resolveReference(input.delegation, handles);
          actual = await apiRequest(app, "/evaluations", "POST", {
            ...input,
      ...
      case "createStepUpRequest": {
        ...
        actual = await apiRequest(app, "/step-up/requests", "POST", {
          decisionLogId: evaluation.logId,
          expiresAt: input.expiresAt,
        });
      ...
      case "resolveStepUp": {
        ...
        actual = await apiRequest(app, `/step-up/requests/${encodeURIComponent(request.id)}/resolve`, "POST", {
      ...
      case "consumeAuthorization": {
        ...
        actual = await apiRequest(app, "/step-up/authorizations/consume", "POST", authorizationBinding(authorization));
        break;
      }
      case "consumeReceipt": {
        const evaluation = resolveReference(input.evaluation, handles) as JsonRecord;
        const decision = evaluation.decision as AccessDecision;
        actual = await apiRequest(app, "/receipts/consume", "POST", {
          receipt: evaluation.receipt,
          expected: receiptExpected(decision),
        });
        break;
      }
      case "getOnboardingView": {
        ...
        actual = await apiRequest(app, `/zkya/onboarding-views/${encodeURIComponent(decisionLogId)}`);
        break;
      }
    }
  ...
  for (const kind of ID_KINDS) assert.deepEqual(queues.get(kind), [], `${transcript.name} left unused ${kind} IDs`);
  return executed;
}
...
test("versioned lifecycle fixture schema rejects unknown and missing fields before API execution", async () => {
...
test("API runner executes every versioned authority lifecycle transcript step", async () => {
  const fixture = await loadFixture();
